...
def get_series(series_name: str, pct_chg=False, pct_chg_freq=1):
    series = pf.get_series(series_id=series_name, api_key=api_key).set_index('date').value.rename(series_name)
    if pct_chg:
...
class DataSet:
    def __init__(self, rid: int, series: list[str], pct_chg: Union[bool, list[bool]] = False):
        self.rid = rid
        self.series = series
    ...
    def get_release_offsets(self):
        ...
        for date in self.data.index:
            pred = self.release_dates.index[self.release_dates.index.get_indexer([date], method='bfill')[0]]
...
class DataCollection:
    ...
    def merge_datasets(self):
        ...
        for dataset in self.datasets[1:]:
            merged = pd.merge(merged, dataset.data, left_index=True, right_index=True, how='outer')
        return merged
...
class FOMCData(DefinedDataSet):
    rid = 326
    date_file_offset = 41
    series = ['FEDTARMD', 'FEDTARMDLR', 'FEDTARRL', 'FEDTARRM', 'FEDTARCTM', 'FEDTARCTH', 'FEDTARRHLR', 'FEDTARRH', 'FEDTARCTLLR']
    pct_chg = False
...
master_df = pd.merge(data.data, market_df, how='outer', left_index=True, right_index=True).ffill()
