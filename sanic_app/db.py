...
if __name__ == "__main__":
    ...
    test_df = (
        pl.read_csv("sanic_app/data/master.csv", try_parse_dates=True)
    ...
    Jobs.add("test_job")
    RawData.chunk("test_table", "test_job", 252)
    print(CVData.read("test_table", "test_job", 0))
    StdCVData.create()
    start = time.time()
    print('SYNC')
    for i in range(CVData.get_cv_no_chunks("test_table", "test_job")):
        CVData.standardize("test_table", "test_job", i)
    ...
    loop.run_until_complete(async_std())
    print(time.time() - start)
    RenderData.drop()
    RenderData.create()
